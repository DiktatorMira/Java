...
@Singleton public class MixedFormParseService implements FormParseService {
    ...
    @Inject public MixedFormParseService() {
    ...
    @Override public FormParseResult parse( HttpServletRequest request ) {
        ...
        if (isMultipart) {
            try {
                for (FileItem item : fileUpload.parseRequest(request)) {
                    if (item.isFormField()) fields.put(item.getFieldName(), item.getString(charset));
                    else files.put(item.getFieldName(), item);
                }
