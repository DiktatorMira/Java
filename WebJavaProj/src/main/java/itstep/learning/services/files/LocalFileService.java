...
@Singleton public class LocalFileService implements FileService {
    private final String uploadPath;

    @Inject public LocalFileService(StringReader stringReader) {
    ...
    @Override public String upload( FileItem fileItem ) {
    ...
    @Override public InputStream download( String fileName ) throws IOException {
        File file = new File( this.uploadPath, fileName );
        if(file.isFile() && file.canRead()) return Files.newInputStream( file.toPath() );
        return null;
    }
}