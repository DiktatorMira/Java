...
public class BaosStringReader implements StringReader {
    @Override public String read(InputStream inputStream) throws IOException {
        ...
        try(ByteArrayOutputStream byteBuilder = new ByteArrayOutputStream(); BufferedInputStream bis = new BufferedInputStream(inputStream)) {
            int len;
            while((len = bis.read(buffer)) != -1) byteBuilder.write(buffer, 0, len);
