...
@Singleton public class CartServlet extends RestServlet {
    private final CartDao cartDao;
    private final ProductDao productDao;

    @Inject public CartServlet(CartDao cartDao, ProductDao productDao) {
    ...
    @Override protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (user != null) super.sendResponse(200, cartDao.getCartByUser(user, true));
        else super.sendResponse(401, null);
    }
    @Override protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            if (cartDao.add(user, product)) super.sendResponse( 201, "Added" );
            else super.sendResponse( 500, "Error adding product" );
        } catch (Exception ex) {
    ...
    @Override protected void doPut(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        String productId = req.getParameter( "product-id" );
        if (productId == null || productId.isEmpty()) super.sendResponse( 400, "Missing required parameter 'product-id'" );

        int increment;
        String delta = req.getParameter( "delta" );
        if (delta == null || delta.isEmpty()) increment = 1;
        else {
        ...
        try {
            if (cartDao.update(cartId, productId, increment)) super.sendResponse(202, "Updated");
            else super.sendResponse(500, "Error updating product");
        } catch (Exception ex) {
            super.sendResponse(400, ex.getMessage());
        }
    }
    @Override protected void doDelete(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        String productId = req.getParameter( "product-id" );
        if (productId == null || productId.isEmpty())  cartDao.deleteCart( cartId );
        else cartDao.deleteCartItem( cartId, productId );
