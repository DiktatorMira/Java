...
@Singleton public class UserDao {
    ...
    @Inject public UserDao(Connection connection, Logger logger, @Named("digest") HashService hashService) {
    ...
    public User getUserById( UUID id ) {
        ...
        try (Statement statement = connection.createStatement()) {
            ...
            if (resultSet.next()) return new User( resultSet );
    ...
    public User authenticate( String login, String password ) {
        ...
        try (PreparedStatement prep = connection.prepareStatement(sql)) {
            ...
            if (res.next()) {
                ...
                if (hashService.digest(salt + password).equals(dk)) return new User(res);
            }
        } catch(SQLException ex) {
            logger.log( Level.WARNING, ex.getMessage() + " -- " + sql, ex );
        }
    ...
    public boolean installTables() {
        ...
        try(Statement stmt = connection.createStatement()) { stmt.executeUpdate( sql ); }
        catch(SQLException ex) {
            logger.log( Level.WARNING, ex.getMessage() + " -- " + sql, ex );
            return false;
        }
