...
public class CategoryDao {
    ...
    @Inject public CategoryDao(@Named("Oracle") DbService dbService, Logger logger) {
    ...
    public List<Category> read() {
        ...
        try (Statement stmt = dbService.getConnection().createStatement()) {
            ...
            while (rs.next()) categories.add(new Category(rs));
            rs.close();
    ...
    public Category create( Category category ) {
        ...
        try (PreparedStatement prep = dbService.getConnection().prepareStatement(sql)) {
            ...
            if (category.getDeleteDt() != null) prep.setTimestamp(5, new Timestamp(category.getDeleteDt().getTime()));
            else prep.setTimestamp(5, null);
            prep.executeUpdate();
            return category;
        } catch( SQLException ex ) {
            logger.warning(ex.getMessage() + " -- " + sql);
