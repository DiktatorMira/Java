...
public class TokenDao {
    ...
    @Inject public TokenDao( Connection connection, Logger logger ) {
    ...
    public User getUserByTokenId( UUID tokenId ) throws Exception {
        ...
        try(PreparedStatement prep = connection.prepareStatement(sql)) {
            ...
            if (rs.next()) {
                ...
                if (token.getExp().before(new Date())) throw new Exception("Token expired") ;
                return new User( rs );
            } else throw new Exception("Token rejected") ;
