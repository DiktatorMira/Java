...
<ul>
    <%
        List<AccessLog> accessLogs = accessLogDao.getLastLogs();  // Пример метода получения последних записей
        for (AccessLog log : accessLogs) {
    %>
    <li><%= log.getUserId() %> заходил на <%= log.getPageUrl() %> в <%= log.getAccessTime() %></li>
    <%
        }
    %>
</ul>